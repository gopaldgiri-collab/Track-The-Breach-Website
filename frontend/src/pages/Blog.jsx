    ...
    <div data-testid="blog-page">
      ...
      {featured && (
        <section className="container mx-auto px-6 lg:px-8 mb-16">
          <a href={`/blog/${featured.slug}`} data-testid="blog-featured" className="block group relative overflow-hidden rounded-2xl card-elev p-8 md:p-12 hover:-translate-y-1 transition">
            <div className="absolute -top-20 -right-20 w-80 h-80 bg-blue-300/30 blur-3xl rounded-full" />
            <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-purple-300/20 blur-3xl rounded-full" />
            <div className="relative">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass border border-blue-200 text-xs font-mono uppercase tracking-[0.18em] text-blue-600">
                Featured · {featured.category}
              </div>
              <h2 className="mt-6 font-display text-3xl md:text-5xl font-bold text-slate-900 tracking-tight max-w-3xl leading-tight">{featured.title}</h2>
              <p className="mt-5 text-base text-slate-600 max-w-2xl">{featured.excerpt}</p>
              <div className="mt-6 flex items-center gap-4 text-xs text-slate-500">
                ...
                <span className="inline-flex items-center gap-1.5 text-blue-600 font-semibold ml-auto">Read article <ArrowRight className="w-3.5 h-3.5" /></span>
              </div>
            </div>
          </a>
        </section>
      ...
      <section className="container mx-auto px-6 lg:px-8 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {rest.map((p, i) => (
            <a key={i} href={`/blog/${p.slug}`} data-testid={`blog-post-${i}`} className="group card-elev rounded-2xl p-6 hover:-translate-y-1 transition flex flex-col">
        ...
        {posts.length === 0 && (
          <div className="text-center py-20 text-slate-500" data-testid="blog-empty">No articles match your search.</div>
        )}
      </section>
    </div>
  );
